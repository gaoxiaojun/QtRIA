...
for(dep, plugin_deps) {
    ...
    dependencyList += "        <dependency name=\"$$QTC_PLUGIN_NAME\" version=\"$$QFOREX_VERSION\"/>"
}
for(dep, plugin_recmds) {
    ...
    isEmpty(dependencies_file): \
        error("Plugin dependency $$dep not found")
    include($$dependencies_file)
    dependencyList += "        <dependency name=\"$$QTC_PLUGIN_NAME\" version=\"$$QFOREX_VERSION\" type=\"optional\"/>"
}
dependencyList += "    </dependencyList>"
...
isEmpty(USE_USER_DESTDIR) {
    ...
    DESTDIR = "$$DESTDIRBASE/QtProject/$$DESTDIRAPPNAME/plugins/$$QFOREX_VERSION"
}
LIBS += -L$$DESTDIR
